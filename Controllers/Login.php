<?php 
	...
	class Login extends Controllers
	{
		...
		{	
			...
			$data['page_tittle']="Tienda Virtual";
				...
				else
				{
					...
					else
					{
						...
						if($arrData['status'] == 1)
						{
							...
							$arrResponse = array('status' => true, 'msg' => 'Acceso concedido');
						}
						else
						{
			...
			if($_POST)
			{
				...
				else
				{
					...
					else
					{
						...
						$request = $this->model->setTokenUser($idpersona,$token);
						...
						if($request)
						{
							...
							if($sendEmail)
							{
								$arrResponse = array('status' => true, 'msg' => 'Se ha enviado un mensaje a tu cuenta de correo para cambiar tu contraseña.');
							}
							else
							{
								$arrResponse = array('status' => true, 'msg' => 'No se ha podido enviar el mensaje a su correo, intentelo más tarde.');
							}
		...
		public function confirmUser(string $params)
		{
			...
			else
			{
				...
				else
				{
					...
					$data['page_tittle']="Cambiar contraseña";
		...
		{
			if(empty($_POST['idUsuario']) ||empty($_POST['txtPassword']) ||empty($_POST['txtToken']) ||empty($_POST['txtPasswordConfirm']))
			{
				$arrResponse = array('status' => false, 'msg' => 'Error de datos');
			}
			else
			{
				...
				$strPasswordConfirm = intval($_POST['txtPasswordConfirm']);
